...
```{r}
t.test(fg_pct ~ modelcheck, data=teamvisdata_fixed)
```

### Linear Regression Testing for Game Statistics

So one of the problems with predicting the outcome of these games is that we don't know what will happen during the game until it's over. For instance, if I know a team is going to shoot 65% from 3PT and their opponent is going to have a season high in turnovers; I would always pick them to win that game regardless of who that is. I can attempt to estimate these statistics and see if they can help increase model accuracy.

There are various statistics that I've identified through a two-sided t-test that are significantly different in games that the model incorrectly predicted. I'm only going to focus on predicting those with the lowest p-values.

Note: Field Goal % is lower than Personal Fouls; however, True Shooting % takes Field Goal % into account when calculated along with additional shooting statistics.
