...
Here I'm going to load in the data we've been working with and do some slight cleaning up of our variables prior to running some regression models.
...
Let's run the model that was the best performing during cross validation. Then let's add the predicted values from the model to our data set to use in some visualizations.

```{r}
winbinary1 <- glm(twoleveloc ~ roll_offrating + roll_defrating + team +
                      homeaway + vorp_bpa + roll_three_par + roll_ftar +
                      roll_ast_pct,
                  data=teamvisdata, family='binomial')

winbinary1_summary <- summary(winbinary1)

winbinary1_fitted <- fitted(winbinary1)

winbinary1_approx <- predict(winbinary1, newdata=teamvisdata, type='response')

teamvisdata2 <- cbind(teamvisdata, winbinary1_approx) %>%
    mutate(winbinary1_approx = round(100 * winbinary1_approx, 1))
```

```{r}
testmodel <- multinom(fourleveloc ~ 1, data = teamvisdata)
summary(testmodel)
```
